...
static void make_dataset (const v8::FunctionCallbackInfo<Value>& args)
{
    ...
    //Atributes
        ...
        Nan::TypedArrayContents<Float32Array> arrayContents(args[2]);
        
        for(unsigned int index=arrayContents.length();index<propertyNames->Length();index++)
        {
...
//        Local<Value> rankValue=buffer->Get(String::NewFromUtf8(v8::Isolate::GetCurrent(), "columns"))->ToInt32()->Value();
    ...
    //Atributes
        ...
        Nan::TypedArrayContents<Float32Array> arrayContents(buffer);
...
static void write_dataset (const v8::FunctionCallbackInfo<Value>& args)
{
    ...
    err=H5LTget_dataset_info(args[0]->ToInt32()->Value(), *dset_name, values_dim.get(), &class_id, &bufSize);
    if(err<0)
    {
...
//                std::cout<<"c side\n"<<buffer<<std::endl;
//                args.GetReturnValue().Set(String::NewFromUtf8(v8::Isolate::GetCurrent(), buffer.c_str(), String::kNormalString, theSize));
                args.GetReturnValue().Set((Nan::NewBuffer((char*)buffer.c_str(), theSize)).ToLocalChecked());
            }
                break;
            case H5T_INTEGER:
            case H5T_FLOAT:
            ...
            std::cout<<"H5Sselect_hyperslab err "<<(err)<<std::endl;
                ...
                v8::Local<v8::Object> buffer=(Nan::NewBuffer(bufSize*theSize)).ToLocalChecked();// node::Buffer::New(v8::Isolate::GetCurrent(),bufSize*theSize);
